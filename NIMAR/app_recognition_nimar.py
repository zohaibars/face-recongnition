...
def processing_image(input_path):
    ...
    if results:
    #     json_data = pd.DataFrame(results).to_json(orient='records')
    # else:
    #     json_data = "[]"
...
def processing_chunk(input_path,chunk_timestamp):
    ...
    try:
            ...
            for i in range(len(bboxs)):
                ...
                if name is None:
                    ...
                    if label.startswith("Unknown"):
                       unknown_person_entry = {
                            'name': None,
                            'timestamps': None,  # Initialize with the current timestamp
                            'thumbnail': None,
                            'coverageTime': None,
                            'startTime': None,  # Initialize with the current timestamp
                            'endTime': None,
                        }
                       unknown_data.append(unknown_person_entry)
                    else:
        ...
        print(f"Error encountered during video processing: {e}")
    if person_data:
...
def main_func(video_file):
    ...
    if video_file.filename.endswith(('.mp4', '.avi', '.mov', '.mkv')):
        json_data = processing_chunk(input_path, chunk_timestamp)
        
        return json_data
    else:
        json_data = processing_image(input_path)
...
async def upload_and_split_video(
    ...
    if not video_file.filename.endswith(('.mp4', '.avi', '.mov', '.mkv', '.jpg', '.png', '.PNG','.jpeg')):
    ...
    if json_data:
        ...
        # Ensure json_data_list is always a list of dictionaries
        if isinstance(json_data_list, dict):
            # Wrap the single dictionary in a list
            json_data_list = [json_data_list]
        elif isinstance(json_data_list, list) and len(json_data_list) == 1 and isinstance(json_data_list[0], dict):
            pass
        else:
            pass

        return JSONResponse(content=json_data_list)
    else:
        return JSONResponse(content={"message": "No data available."})

@app.get("/handshake/")
async def handshake():
    return JSONResponse(content={"handshake": "success", "token": "your_token_here"})

if __name__=="__main__":
    # while True:
    uvicorn.run("app_recognition_nimar:app", host="192.168.18.80", port=9000, reload= True)
