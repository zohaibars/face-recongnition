        ...
        while cap.isOpened():
            ...
            for i in range(len(bboxs)):
                ...
                if name is None:
                    ...
                    #cv2.rectangle(frame, (x1, y1), (x1 + t_size[0], y1 + t_size[1]), (0, 146, 230), -1)
                    #cv2.putText(frame, caption, (x1, y1 + t_size[1]), cv2.FONT_HERSHEY_PLAIN, 2, [255, 255, 255], 2)
                    #count=len(os.listdir("output_videos"))+1
                    #cv2.imwrite(f"output_videos/frame_{count}.jpg", face_image)
...
if __name__=="__main__":
    # while True:
    uvicorn.run("app_without_audio_extraction_mongo_db:app", host="192.168.18.80", port=8008, reload= True)
