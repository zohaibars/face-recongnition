...
def processing_chunk(input_path, chunk_timestamp):
    ...
    try:
        ...
        while cap.isOpened():
            ...
            #db.add_name_embedding(unknown_query_embs)
...
def main_func(video_file):
    ...
    # video_base_name, video_extension = os.path.splitext(video_file.filename)
    # print("video_base_name",video_base_name)
    # if video_chunk.endswith(".mp4"):
    # if video_chunk.startswith(video_base_name):
    ...
    processing_chunk(input_path, chunk_timestamp)
...
async def upload_and_split_video(
    ...
    # output_video=os.path.basename(video_file)
    if 'json_data' in globals() and json_data:
        #video = "output_videos"
        #video_base64 = encode_video_to_base64(f"{video}/{video_file.filename}")
        response_data = {"data": json_data}
        #response_data = {"data": json_data, "video_base64": video_base64}
        # print("\n video_base64\n\n",video_base64 )
...
if __name__=="__main__":
    # while True:
    uvicorn.run("app_without_audio_extraction:app", host="192.168.18.80", port=8002, reload= True)
